...
using namespace std;
void Reassembler::insert( uint64_t __first_index, string data, bool __is_last_substring )
{
  if ( output_.writer().is_closed() )
    return;
  const uint64_t _1 = output_.writer().bytes_pushed(), _2 = output_.writer().available_capacity(), _3 = data.size();
  ...
  uint64_t l = max( __first_index, _1 ), r = min( __first_index + _3, _1 + _2 ), _l = l - __first_index,
           len = r - l;
